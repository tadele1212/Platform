...
import websockets
import json
import pyaudio  # For audio handling
import pyttsx3
...
class DialogueSystem:
    ...
    async def stream_audio(self, text):
        try:
            ...
            voice = "nova" if self.current_npc == "HR" else "alloy"  # nova for HR (female), alloy for CEO (male)
            
            # Use the OpenAI client directly instead of WebSocket
            ...
            print(f"[DialogueSystem] Audio streaming error: {e}")
        ...
        try:
            ...
            for chunk in response:
                if chunk.choices[0].delta.content:
                    # Append each new chunk to the message
                    self.npc_message += chunk.choices[0].delta.content
                    print(f"[DialogueSystem] Chunk received: {chunk.choices[0].delta.content}")
                    
                    # Update display in real-time
                    self.render()
                    pygame.display.flip()
                    
                    # Small delay for readable text flow
                    pygame.time.wait(30)

            # Store complete message in conversation history
            self.conversation_history.append({
                "role": "assistant",
                "content": self.npc_message
            })
            
            print(f"[DialogueSystem] NPC complete message: {self.npc_message}")
            
            # Create and run a new event loop for audio
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.run_until_complete(self.stream_audio(self.npc_message))
            loop.close()
    ...
    def handle_input(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_v:  # 'V' key to toggle voice input
                if self.audio_input.is_listening:
        ...
        while not self.audio_input.audio_queue.empty():
            voice_input = self.audio_input.audio_queue.get()
            self.user_input = voice_input
            # Simulate Enter key press to send the message
            return self.handle_input(pygame.event.Event(pygame.KEYDOWN, {'key': pygame.K_RETURN}))
...
class AudioInputManager:
    def __init__(self):
        ...
        print("[AudioInput] Calibrating for ambient noise...")
        with self.microphone as source:
            self.recognizer.adjust_for_ambient_noise(source)
        print("[AudioInput] Calibration complete")
        
    def start_listening(self):
        if not self.is_listening:
            self.is_listening = True
            self.recognition_thread = threading.Thread(target=self._listen_loop)
    ...
    def _listen_loop(self):
        while self.is_listening:
            try:
                with self.microphone as source:
                    print("[AudioInput] Listening for speech...")
                    audio = self.recognizer.listen(source, timeout=1, phrase_time_limit=5)
                    
                try:
                    text = self.recognizer.recognize_google(audio)
                    print(f"[AudioInput] Recognized: {text}")
                    self.audio_queue.put(text)
                except sr.UnknownValueError:
                    print("[AudioInput] Could not understand audio")
                except sr.RequestError as e:
                    print(f"[AudioInput] Error with speech recognition service; {e}")
                    
            except Exception as e:
                print(f"[AudioInput] Error in listen loop: {e}")
                if not self.is_listening:
                    break
...
class Game3D:
    ...
    def run(self):
        ...
        try:
            ...
            # Add cleanup for audio input
            if hasattr(self.dialogue, 'audio_input'):
                self.dialogue.audio_input.stop_listening()
            pygame.quit()
